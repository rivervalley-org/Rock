...
{
    [DisplayName("Prayer Cards Single Campus Record Generator")]
    [Category("org_rivervalley > PrayerCards")]
    [Description("Prayer Cards for a single campus ")]

    public partial class SingleCampusRecordGenerator : Rock.Web.UI.RockBlock
    {
        #region Fields
        ...
        protected void btnDetail_Click(object sender, EventArgs e)
        {
            ...
            { 
                ...
                if (nonfamilyRecordsExist == true && familyRecordsExist == true)
                {
                    formError = "bothExist";
                    nbWarningMessage.Visible = true;
                    nbWarningMessage.Text += "<br>Records for the both Non-Family groups and Families aready exist for this date.  No action was taken";
                }
                else
                {
                    if (familyRecordsExist == true)
                    {
                        formError = "oneExists";
                        nbWarningMessage.Visible = true;
                        nbWarningMessage.Text += "<br>Records for the Families/Life Groups for this campus aready exist for this date. No new records were created";
                    }
                    else
                    {
                        ...
                        lOutputString.Text += "<b>Family/Life Groups records were created per this schedule:</b>";
                    ...
                    if (nonfamilyRecordsExist == true)
                    {
                        formError = "oneExists";
                        nbWarningMessage.Visible = true;
                        nbWarningMessage.Text += "<br>Records for the Non-Family groups aready exist for this date. No new records were created";
                    }
                    else
                    {
                        SetSelectedCampus(campusId);
                        GetCampusEntityId();
                        GetAttribute296Details(); // gets appropriate number of prayer records for each type prayer category
                        lOutputString.Text += "<hr><b>Non family group records were created per this schedule:</b>";
                ...
                if (formError == "bothExist")
                {
                    pnlDetail.Visible = true;
                    pnlError.Visible = true;
                    pnlComplete.Visible = false;
                }
                if (formError == "oneExists")
                {
                    pnlDetail.Visible = false;
                    pnlError.Visible = true;
                    pnlComplete.Visible = true;
        ...
        private void GetCampusEntityId()
        {
            ...
            avQry = avQry.Where(row => row.AttributeId == 6227 && row.Value == cGuid);
        ...
        private void ProcessFamilies(int count)
        {
            ...
            {                
                ...
                aliasId = cPerson.PrimaryAliasId ?? default(int);
                BuildPersonPrayerCardNames(cPerson.Id, "family");
        ...
        private void ProcessVirtues(int count)
        {
        ...
        {
            PrayerDatesDetail attibuteDetail;
            var rockContext = new PrayerDatesContext();
            var service = new PrayerDatesDetailService(rockContext);

            attibuteDetail = new PrayerDatesDetail();
            service.Add(attibuteDetail);
            attibuteDetail.PersonId = personId;
            attibuteDetail.PersonAliasId = aliasId;
            attibuteDetail.Type = type;
            attibuteDetail.Names = names;
            attibuteDetail.DatePrayedFor = datePrayed;
            if (nullCampus == false)
            {
                attibuteDetail.CampusId = campus;
            }
            else
            {
                attibuteDetail.CampusId = null;
            }
            rockContext.SaveChanges();
