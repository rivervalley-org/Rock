...
{
    [DisplayName("Prayer Cards Record Generator")]
    [Category("org_rivervalley > PrayerCards")]
    [Description("Prayer Cards generator for either single or multicampus use. ")]

    public partial class PrayerCardRecordGenerator : Rock.Web.UI.RockBlock
    {
        #region Fields
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                string qsParameter = PageParameter("type");
                if (!string.IsNullOrWhiteSpace(qsParameter))
                {
                    if (qsParameter == "single")
                    {
                        formType = "single";
                        pnlSingleCampusDetail.Visible = true;
                    ...
                    {
                        if (qsParameter == "multi")
                        {
                            formType = "multi";
                            pnlSingleCampusDetail.Visible = false;
                        ...
                        else
                        {
                            ...
                            nbWarningMessage.Text += "There has been an error in processing your request";
                        }                        
                    }
                }
                else
        ...
        protected void btnSingleCampusDetail_Click(object sender, EventArgs e)
        {
            pnlError.Visible = false;
            pnlSingleCampusDetail.Visible = false;
            pnlMultiCampusDetail.Visible = false;
            ...
            datePrayedFor = Convert.ToDateTime(dtPickerSingle.SelectedDate.ToString());
            ...
            else
            {
                ...
                if (churchRecordsExist == true && campusRecordsExist == true)
                {
                    formErrorType = "bothExist";
                    nbWarningMessage.Visible = true;
                    nbWarningMessage.Text += "<br>Both Church and Campus records aready exist for this date.  No action was taken";
                }
                else
                {
                    if (campusRecordsExist == true)
                    {
                        formErrorType = "oneExists";
                        nbWarningMessage.Visible = true;
                        nbWarningMessage.Text += "<br>Campus records already exist for this date. No new campus records were created";                      
                    }
                    else
                    {
                        ...
                        lOutputString.Text += "<b>Campus records were created:</b>";
                    ...
                    if (churchRecordsExist == true)
                    {
                        formErrorType = "oneExists";
                        nbWarningMessage.Visible = true;
                        nbWarningMessage.Text += "<br>Church records aready exist for this date. No new records were created";
                    }
                    else
                    {
                        SetSelectedCampus(campusId);
                        GetCampusEntityId();
                        GetAttribute296Details(); // gets appropriate number of prayer records for each type prayer category
                        lOutputString.Text += "<hr><b>Church records were created:</b>";
                ...
                if (formErrorType == "bothExist")
                {
                    pnlSingleCampusDetail.Visible = true;
                    pnlError.Visible = true;
                    pnlComplete.Visible = false;
                }
                if (formErrorType == "oneExists")
                {
                    pnlSingleCampusDetail.Visible = false;
                    pnlError.Visible = true;
                    pnlComplete.Visible = true;
        ...
        protected void btnMultiCampusDetail_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (churchRecordsExist == true)
                {
                    formError = true;
                    nbWarningMessage.Visible = true;
                    nbWarningMessage.Text += "<br>Church records already exist for this date. No new records were created";
                }
                else
                {
                    ...
                    GetCampusEntityId();
                    GetAttribute296Details(); // gets appropriate number of prayer records for each type prayer category
                    lOutputString.Text += "<b>Church records were created:</b>";
                    BuildNonFamilyOutputString();
                    ProcessNetworkChurches(networkChurchesCount);
                    ProcessLocalChurches(localChurchesCount);
                    ProcessMinistryLeaders(ministryLeaderCount);
                ...
                {
                    debugCounter++;
                    lDebugString.Text += "<br>Debug counter " + debugCounter;
                    if (item.Selected)
                    {
                        ...
                        if (campusRecordsExist == true)
                        {
                            formError = true;
                            nbWarningMessage.Visible = true;
                            nbWarningMessage.Text += "<br>Campus records for " + campusName + " aready exist for this date. No new records were created";
                        }
                        else
                        {
                            lOutputString.Text += "<hr>";
                ...
                lDebugString.Text += "<br> Debug Counter2 " + debugCounter2;
        ...
        private void CheckForExistingPrayerDate()
        {
            ...
            try
            {
                if(formType == "single")
                {
                    parameterDTValue = DateTime.Parse(dtPickerSingle.SelectedDate.ToString());
                }
                else
                {
        ...
        protected void BuildCampusDropDown()
        {
            ...
            ddlList.DataSource = new CampusService(new RockContext()).Queryable()
                .Where(x => x.Id == 1 || x.Id == 2 || x.Id == 3 || x.Id == 4 || x.Id == 9 || x.Id == 11 || x.Id == 12 || x.Id == 13)
