...
{
    [DisplayName("Prayer Cards Record Processing")]
    [Category("org_rivervalley > PrayerCards")]
    [Description("Processes records selected in selection block ")]

    public partial class PrayerCardsRecordProcessing : Rock.Web.UI.RockBlock
    {
        #region Fields
        ...
        {
            qsDate = PageParameter("date");
            qsCampuses = PageParameter("campusIds");
            string[] ids = qsCampuses.Split(',');
            foreach (var item in ids)
            {
                if (item != "")
                {
                    var id = Int32.Parse(item);
                    selectedCampusIds.Add(id);
                }
            }
        }
        ...
        protected void ProcessRecords()
        {
            ...
            {
                parameterDTValue = DateTime.Parse(qsDate.ToString());
                dateNull = false;
            }
            ...
            if (dateNull == false)
            {
                datePrayedFor = Convert.ToDateTime(qsDate.ToString());
                ...
                else
                {
                    ...
                    {
                        lOutputString.Text = "Church records already exist for this date.";
                    }
                    else
                    {
                    ...
                    foreach (int scItem in selectedCampusIds)
                    {
                        ...
                        {
                            lOutputString.Text += "<br>Campus records for " + campusName + " aready exist for this date.";
                        }
                        else
                        {
                            /*
            ...
            {
                lOutputString.Text += "<br>An error occurred in processing your request.";
            }
        }
