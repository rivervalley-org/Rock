...
/// <summary>
/// Custom Group Registration that includes the following features:
///     - Asks for birthdate
///     - Asks for marital status if set to 'Full' mode
///     - Displays group member attributes as fields  
/// </summary>
    ...
    public partial class GroupRegistration : RockBlock
    {
        ...
        protected void btnRegister_Click( object sender, EventArgs e )
        {
            ...
            if ( Page.IsValid && _isValidSettings )
            {
                ...
                else
                {
                    ...
                    // update birthdate
                    if ( bpBirthdate.SelectedDate.HasValue )
                    {
                        person.SetBirthDate( bpBirthdate.SelectedDate );
                    }
                ...
                if ( !IsSimple )
                {
                    ...
                    if ( IsFullWithSpouse && tbSpouseFirstName.Text.IsNotNullOrWhiteSpace() && tbSpouseLastName.Text.IsNotNullOrWhiteSpace() )
                    {
                            ...
                            !tbSpouseLastName.Text.Trim().Equals( spouse.LastName.Trim(), StringComparison.OrdinalIgnoreCase ) )
                        {
                            ...
                            if ( bpSpouseBirthdate.SelectedDate.HasValue )
                            {
                                spouse.SetBirthDate( bpSpouseBirthdate.SelectedDate );
                            }
        ...
        private void ShowDetails()
        {
            ...
            if ( _group != null )
            {
                ...
                bpBirthdate.Required = GetAttributeValue( REQUIRE_BIRTHDATE_KEY ).AsBoolean();

                pnlHomePhone.Visible = !IsSimple;
                pnlCellPhone.Visible = !IsSimple;
                acAddress.Visible = !IsSimple;
                bpBirthdate.Visible = !IsSimple;
                ...
                if ( CurrentPersonId.HasValue && _autoFill )
                {
                    ...
                    if ( !IsSimple )
                    {
                        ...
                        {
                            bpBirthdate.SelectedDate = CurrentPerson.BirthDate;
                        }
