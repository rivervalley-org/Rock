...
namespace RockWeb.Plugins.org_riverValley.Prayer
{
    [DisplayName( "Prayer Session" )]
    [Category( "River Valley > Prayer" )]
    ...
    [BooleanField( "Public Only", "If enabled, only public prayer requests will be displayed.", false, "Filtering", 3 )]
...
" )]
    ...
    public partial class PrayerSession : RockBlock
    {
        ...
        private bool BindCategories( string categoryGuid )
        {
            ...
            if ( GetAttributeValue("PublicOnly").AsBoolean() )
            {
                prayerRequestQuery = prayerRequestQuery.Where( p => p.IsPublic.HasValue && p.IsPublic.Value );
            }
