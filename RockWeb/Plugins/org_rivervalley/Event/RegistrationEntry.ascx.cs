...
namespace RockWeb.Plugins.org_riverValley.Event
{
    ...
    [IntegerField( "Session Minutes", "The maximum number of minutes a user should be allowed for completing a registration when there is a limit to number of registrants (Set to 0 for unlimited).", false, 0, "", 11 )]
    [BooleanField( "Default To Minimum Amount Due", "Should the default payment amount default to the minimum amount due?", false, "", 12 )]
    ...
    public partial class RegistrationEntry : RockBlock
    {
        ...
        private const string SESSION_EXPIRATION_KEY = "SessionExpiration";
        ...
        /// <summary>
        /// Gets or sets the time that the current session should expire
        /// </summary>
        /// <value>
        /// The time that current session should expire
        /// </value>
        private DateTime? SessionExpiration { get; set; }
        ...
        protected override void LoadViewState( object savedState )
        {
            ...
            SessionExpiration = ViewState[SESSION_EXPIRATION_KEY] as DateTime?;
                        ...
                        if ( RegistrationTemplate != null )
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        // If the registration has a limited number of slots available, and a session timeout has been configured, set the expiration time
                                        int sessionMinutes = GetAttributeValue( "SessionMinutes" ).AsInteger();
                                        if ( sessionMinutes > 0 )
                                        {
                                            SessionExpiration = RockDateTime.Now.AddMinutes( sessionMinutes );
                                        }
        ...
        protected override object SaveViewState()
        {
            ...
            ViewState[SESSION_EXPIRATION_KEY] = SessionExpiration;
        ...
        /// <summary>
        /// Validates that session has not yet expired
        /// </summary>
        private bool IsSessionValid()
        {
            if ( SessionExpiration.HasValue && SessionExpiration.Value < RockDateTime.Now )
            {
                string url = ResolvePublicUrl( Request.Url.PathAndQuery );
                ShowError( "Session Expired", string.Format( "Your registration session has expired. Please <a href='{0}'>start your registration over</a>.", url ) );
                return false;
            }
        ...
        private void ShowInfo( string heading, string text )
        {
        ...
        /// <param name="heading">The heading.</param>
        /// <param name="text">The text.</param>
        private void ShowMessage( string heading, string text )
        {
        ...
        private void CreateSummaryControls( bool setValues )
        {
            ...
            if ( setValues && RegistrationState != null && RegistrationInstanceState != null )
            {
                ...
                if ( costs.Where( c => c.Cost > 0.0M ).Any() )
                {
                    ...
                    // if payment amount has not been entered or is less than the minimum payment, set to balance due or minimum due (depending on block setting)
                    if ( !RegistrationState.PaymentAmount.HasValue ||
                        RegistrationState.PaymentAmount.Value < minimumPayment.Value )
                    {
                        if ( GetAttributeValue( "DefaultToMinimumAmountDue" ).AsBoolean() )
                        {
                    ...
                    // if payment amount is more than balance due, set to the balance due
                    if ( RegistrationState.PaymentAmount.Value > balanceDue )
                    {
                        RegistrationState.PaymentAmount = balanceDue;
                    }
