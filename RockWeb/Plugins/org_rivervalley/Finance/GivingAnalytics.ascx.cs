...
", btnCopyToClipboard.ClientID );
            ...
            gAccountSummary.DataKeyNames = new string[] { "Guid" };
            gAccountSummary.ShowActionRow = true;
            gAccountSummary.Actions.ShowExcelExport = true;
            gAccountSummary.GridRebind += gAccountSummary_GridRebind;
        }
        ...
        /// <summary>
        /// Handles the GridRebind event of the gAccountSummary control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void gAccountSummary_GridRebind( object sender, EventArgs e )
        {
            BindGiversGrid();
        }        
...
";
        ...
        private void BindGiversGrid()
        {
            ...
            if ( gGiversGifts.SortProperty != null )
            {
                if ( gGiversGifts.SortProperty.Property.StartsWith( "Account" ) )
                {
                    ...
                    if ( accountId.HasValue )
                    {
                        ...
                        {
                            list = personInfoList.OrderBy( p => p.SortAmount ).ToList();
                        }
                        else
                        {
                            list = personInfoList.OrderByDescending( p => p.SortAmount ).ToList();
                        }
                    }
                    else
                    {
                        list = qry.OrderBy( p => p.LastName ).ThenBy( p => p.NickName ).ToList();
                    }
                }
                else
                {
                    list = qry.Sort( gGiversGifts.SortProperty ).ToList();
                }
            }
            else
            {
            ...
            gGiversGifts.DataSource = list;
            gGiversGifts.DataBind();

            var accountTotals = list
                .SelectMany( i => i.AccountAmounts )
                .GroupBy( a => a.Key )
                .ToDictionary( k => k.Key, v => (decimal?)v.Sum( d => d.Value ) );
            var txnAccountIds = accountTotals.Keys.ToList();
            var accountSummaryList = new FinancialAccountService( rockContext )
                .Queryable().AsNoTracking()
                .Where( a => txnAccountIds.Contains( a.Id ) )
                .ToList()
                .Select( a => new AccountInfo()
                {
                    ...
                    Name = a.Name,
                    Order = a.Order,
                    TotalAmount = accountTotals[a.Id]
                } ).ToList();

            var grandTotalAmount = ( accountSummaryList.Count > 0 ) ? accountSummaryList.Sum( a => a.TotalAmount ?? 0 ) : 0;
            accountSummaryList.Add( new AccountInfo()
            {
                Guid = Guid.NewGuid(),
                Name = "<span class='pull-right'><strong>Total</strong></span>",
                Order = int.MaxValue,
                TotalAmount = grandTotalAmount
            } );

            gAccountSummary.DataSource = accountSummaryList.OrderBy( a => a.Order ).ToList();
