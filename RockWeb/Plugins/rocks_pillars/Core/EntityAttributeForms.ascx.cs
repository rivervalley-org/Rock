...
{
    /// <summary>
    /// Block to display dynamic forms for editing the attribute values associated with a particular context entity.
    /// </summary>
    [DisplayName( "Entity Attribute Forms" )]
    [Category( "Pillars > Core" )]
    [Description( "Block to display dynamic forms for editing the attribute values associated with a particular context entity." )]

    // Block Properties

    // Settings
    [EntityTypeField( "Entity Type", "The type of entitiy that is being updated and who's attributes should be displayed.", true, "CustomSetting", 0 )]
    [TextField("Entity Type Qualifier Column Name", "Optional entity type qualifier column.", false, "", "CustomSetting", 1, "QualifierColumn" )]
    [TextField( "Entity Type Qualifier Value", "Optional entity type qualifier value.", false, "", "CustomSetting", 2, "QualifierValue" )]
    ...
    [TextField( "Done Url", "The url to redirect to when done ('Entity' is available as Lava merge field) <span class='tip tip-lava'></span>.", false, "", "CustomSetting" )]
    [TextField( "Forms", "The forms to show.", false, "", "CustomSetting" )]

    [ContextAware]
    public partial class EntityAttributeForms : RockBlockCustomSettings
    {
        #region Fields
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            {
                nbMain.Title = "Sorry";
                nbMain.Text = ContextTypesRequired.Any() ?
                    "<p>Could not determine the entity to use. Make sure the url contains the correct Context Parameter that is defined in Page Settings dialog (under Advanced Settings).</p>" :
                    "<p>Context Entity Type has not been configured. Use the block settings dialog to set the type of entity, and the Page Settings dialog to define the context parameter name (under Advanced Settings).</p>";
                ...
                if ( entity != null )
                {
                    using ( var rockContext = new RockContext() )
                    {
                        ...
                        foreach ( var keyVal in AttributeValueState )
                        {
                            ...
                            {
                                entity.SetAttributeValue( attribute.Key, keyVal.Value );
                            }
                        }
                        ...
                        if ( CurrentPageIndex >= FormState.Count )
                        {
                            ...
                            if ( workflowTypeGuid.HasValue )
                            {
                                ...
                                if ( workflowType != null && ( workflowType.IsActive ?? true ) )
                                {
                                    try
                                    {
                                        var workflow = Workflow.Activate( workflowType, entity.ToStringSafe() );
                            ...
                            string doneUrl = GetAttributeValue( "DoneUrl" );
                            if ( doneUrl.IsNotNullOrWhiteSpace() )
                            {
                                var mergeFields = LavaHelper.GetCommonMergeFields( RockPage, CurrentPerson );
                                mergeFields.Add( "Entity", entity );
                                string url = doneUrl.ResolveMergeFields( mergeFields );
        ...
        {
            SetAttributeValue( "QualifierColumn", tbEntityTypeQualifierColumn.Text );
            SetAttributeValue( "QualifierValue", tbEntityTypeQualifierValue.Text );
            ...
            SetAttributeValue( "DoneUrl", tbDoneUrl.Text );
        ...
        void tfeForm_DeleteFormClick( object sender, EventArgs e )
        {
            ...
            var attributeFormEditor = sender as EntityAttributeFormEditor;
        ...
        protected void dlgField_SaveClick( object sender, EventArgs e )
        {
            ...
            if ( form != null )
            {
                ...
                field.AttributeId = ddlEntityAttributes.SelectedValueAsInt();
                field.ShowCurrentValue = cbUseEntityCurrentValue.Checked;
                ...
                if ( entity != null )
                {
                    ...
                    foreach ( var form in FormState )
                    {
                                ...
                                a.ShowCurrentValue == true ) )
                        {
                            ...
                            {
                                AttributeValueState.AddOrReplace( field.AttributeId.Value, entity.GetAttributeValue( attributeCache.Key ) );
                            }
                        }
                    }
                }
        ...
        protected override void ShowSettings()
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                lEntityType.Text = ContextTypesRequired.Any() ? ContextTypesRequired.First().FriendlyName : "[Use Block Settings to set Context Entity Type]";
                tbEntityTypeQualifierColumn.Text = GetAttributeValue( "QualifierColumn" );
                tbEntityTypeQualifierValue.Text = GetAttributeValue( "QualifierValue" );
            ...
            tbDoneUrl.Text = GetAttributeValue( "DoneUrl" );
        ...
        {
            var control = new EntityAttributeFormEditor();
        ...
        {
            int order = 0;
            foreach ( var formEditor in phForms.Controls.OfType<EntityAttributeFormEditor>() )
            {
        ...
        private void ShowFormFieldEdit( Guid formGuid, Guid formFieldGuid )
        {
            ...
            if ( form != null )
            {
                ...
                ddlEntityAttributes.Items.Clear();

                var entityTypes = ContextTypesRequired;
                if ( entityTypes.Any() )
                {
                    var type = entityTypes.First().GetEntityType();
                    if ( type != null )
                    {
                        var iHasAttribues = (IHasAttributes)Activator.CreateInstance( type );
                        if ( iHasAttribues != null )
                        {
                            var qualifierCol = tbEntityTypeQualifierColumn.Text;
                            var qualifierValue = tbEntityTypeQualifierValue.Text;
                            if ( qualifierCol.IsNotNullOrWhiteSpace() && qualifierValue.IsNotNullOrWhiteSpace() )
                            {
                                var propertyInfo = type.GetProperty( qualifierCol );
                                if ( propertyInfo != null )
                                {
                                    try { propertyInfo.SetValue( iHasAttribues, Convert.ChangeType( qualifierValue, propertyInfo.PropertyType ), null ); }
                                    catch { }
                                }
                            }
                            iHasAttribues.LoadAttributes();
                            foreach ( var attr in iHasAttribues.Attributes
                                .OrderBy( a => a.Value.Name )
                                .Select( a => a.Value ) )
                            {
                                if ( attr.IsAuthorized( Authorization.VIEW, CurrentPerson ) )
                                {
                                    ddlEntityAttributes.Items.Add( new ListItem( attr.Name, attr.Id.ToString() ) );
                                }
                            }
                        }
                    }
                ...
                ddlEntityAttributes.SetValue( field.AttributeId );

                cbRequireInInitialEntry.Checked = field.IsRequired;
                cbUseEntityCurrentValue.Checked = field.ShowCurrentValue;
        ...
        /// <summary>
        /// Gets the current context entity.
        /// </summary>
        /// <returns></returns>
        private IHasAttributes GetEntity()
        {
            if ( ContextTypesRequired.Count == 1 )
            {
                return this.ContextEntity( ContextTypesRequired.First().Name ) as IHasAttributes;
            }

            return this.ContextEntity() as IHasAttributes;
        }
...
",
            ...
            ScriptManager.RegisterStartupScript( Page, Page.GetType(), "EntityAttributeForms", script, true );
    ...
    [ToolboxData( "<{0}:EntityAttributeFormEditor runat=server></{0}:EntityAttributeFormEditor>" )]
    public class EntityAttributeFormEditor : CompositeControl, IHasValidationGroup
    {
        ...
        /// <summary>
        /// Gets or sets a value indicating whether this <see cref="EntityAttributeFormEditor"/> is expanded.
        /// </summary>
...
";
            ...
            ScriptManager.RegisterStartupScript( this, this.GetType(), "EntityAttributeFormEditorScript", script, true );
        }
