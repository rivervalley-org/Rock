...
", "Formatting", 9)]
    ...
    [TextField("Card Item Class", "Class to use on the div element that wraps each card item.", false, "col-md-4 col-sm-6 col-xs-12", "Formatting", 10 )]
...
", "Formatting", 11 )]
    ...
    public partial class ReservationEntry : Rock.Web.UI.RockBlock
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                var modalTitle = GetAttributeValue( "ModalTitle" );
                var modalContent = GetAttributeValue( "ModalContent" );
                if ( modalTitle.IsNotNullOrWhiteSpace() )
                {
        ...
        private void RptrCards_ItemCommand( object source, RepeaterCommandEventArgs e )
        {
            ...
            {
                DateSelected = true;
                hfDate.Value = datetime.Value.ToString( "o" );
                BindLocations();
                return;
            }
        ...
        private void BindCampuses()
        {
            ...
            if ( campuses.Any() )
            {
                ...
                {
                    lCardCaption.Text = GetAttributeValue( "CampusCaption" );
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( campus != null )
                {
                    ...
                    if ( validKeys.Any() )
                    {
                        ...
                        {
                            hfDate.Value = validKeys.First().ToString().AsDateTime().Value.ToString( "o" );
                            BindLocations();
                        }
                        else
                        {
                            ...
                            lCardCaption.Text = GetAttributeValue( "DateCaption" ).ResolveMergeFields( mergeFields );
                ...
                if ( campus != null && date.HasValue )
                {
                    ...
                    if ( locations.Any() )
                    {
                        ...
                        else
                        {
                            ...
                            lCardCaption.Text = GetAttributeValue( "LocationCaption" ).ResolveMergeFields( mergeFields );
        ...
        private void BindSchedules()
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( campus != null && location != null )
                {
                    ...
                    lCardCaption.Text = GetAttributeValue( "ScheduleCaption" ).ResolveMergeFields( mergeFields );
                    ...
                    if ( schedules.Count == 1 && GetAttributeValue( "SingleScheduleSkip" ).AsBoolean() )
                    {
                        ...
                        {
                            rptrCards.DataSource = schedules.Select( s => new CardItem( s, validOptions ) ).OrderBy( i => i.Order );
                            rptrCards.DataBind();
                        }
                    }
                    else
        ...
        protected string FormatCard( object item )
        {
            ...
            if ( cardItem != null )
            {
				...
				// custom for RVC
				if ( hfCampusGuid.Value.IsNotNullOrWhiteSpace() )
				{				
					mergeFields.Add( "SelectedCampus", CampusCache.Get( hfCampusGuid.Value ) );
				}
                ...
                {
                    mergeFields.Add( "Campus", campus );
                    return GetAttributeValue( "CampusCard" ).ResolveMergeFields( mergeFields );
                }
                ...
                if ( dateItem.HasValue )
                {
                    ...
                    return GetAttributeValue( "DateCard" ).ResolveMergeFields( mergeFields );
                }
                ...
                {
                    mergeFields.Add( "Location", location );
                    return GetAttributeValue( "LocationCard" ).ResolveMergeFields( mergeFields );
                }
                ...
                if ( schedule != null )
                {
                    ...
                    return GetAttributeValue( "ScheduleCard" ).ResolveMergeFields( mergeFields );
                }
            }
        ...
        public class CardItem
        {
            ...
            public CardItem( Schedule schedule, List<OptionItem> options )
            {
                Key = schedule.Guid.ToString();
                Item = schedule;

                var nextStart = schedule.GetNextStartDateTime( RockDateTime.Now );
