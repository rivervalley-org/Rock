...
namespace Plugins.com_bricksandmortarstudio.CheckInExtensions
{
    ...
    [IntegerField("Number of Weeks Back to Display", "How many week backs should headcount entry be displayed for", true, 4, key:"WeeksBack")]
                        ...
                        foreach ( var schedule in location.Schedules.Where( s => s.HasSchedule() ) )
                        {
                            foreach (
                                var startDateTime in
                                    schedule.GetScheduledStartTimes( RockDateTime.Now.AddDays( -7 * weeksBack ), RockDateTime.Now ) )
                            {
