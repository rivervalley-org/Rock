...
namespace RockWeb.Blocks.Cms
{
    ...
    /// </summary>
    [DisplayName("Public Profile Edit RVC Anniversary")]
    [Category("CMS")]
    ...
    public partial class PublicProfileEdit_rvc_anniv : RockBlock
    {
        #region Properties
                ...
                if (group != null)
                {
                    rockContext.WrapTransaction(() =>
                    {
                        ...
                        if (personId == 0)
                        {
                            ...
                            // RVC  Custom Start
                            DateTime? annivdate = dpAnniversaryDate.SelectedDate;
                            if (annivdate.HasValue)
                            {
                                // If setting a future annivdate, subtract a century until annivdate is not greater than today.
                                var today = RockDateTime.Today;
                                while (annivdate.Value.CompareTo(today) > 0)
                                {
                                    annivdate = annivdate.Value.AddYears(-100);
                                }
                            }
                            groupMember.Person.AnniversaryDate = annivdate;
                        ...
                        if (person != null)
                        {
                            ...
                            person.AnniversaryDate = annivdate;
